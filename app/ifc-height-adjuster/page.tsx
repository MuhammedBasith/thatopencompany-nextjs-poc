...
export default function IFCHeightAdjusterPage() {
  ...
  const initialModelHeightRef = useRef<number>(0);
  ...
  const loadIFCFile = async (fileName: string) => {
    ...
    try {
      ...
      if (world) {
        ...
        // Store reference to current model and initial height
        currentModelObjectRef.current = model.object;
        initialModelHeightRef.current = model.object.position.y;
  ...
  const adjustHeight = (delta: number) => {
    if (!currentModelObjectRef.current || !fragmentsRef.current) return;

    const newHeight = heightAdjustment + delta;
    setHeightAdjustment(newHeight);

    // Update the model's Y position
    currentModelObjectRef.current.position.y = initialModelHeightRef.current + newHeight;

    // Update fragments to reflect the change
              ...
              <button
                onClick={() => {
                  ...
                  if (currentModelObjectRef.current) {
                    currentModelObjectRef.current.position.y = initialModelHeightRef.current;
                    fragmentsRef.current?.update(true);
                  }
                }}
                style={{
